...
func (p *Postgres) RegisterUser(email string, password string, userType string, balance int64) (int64, error) {
	...
	stmt := `INSERT INTO users (email, password, type,created_at,last_enter,balance) VALUES ($1, $2, $3, NOW(), NOW(), $4) RETURNING userID`
	var userID int64
...
func (p *Postgres) LoginUser(email string, password string) (int64, error) {
	ctx := context.Background()
	stmt := `SELECT userID FROM users WHERE email=$1 AND password=$2`
